...
# AT override: registered as a filter because AT's
# templates/djangocms_picture/default/picture.html calls
# `instance|thumbnail_dims`. In DE this is an internal helper only, used by
# get_responsive_plugin_image, so the filter never existed and every picture
# rendered through that template raised "Invalid filter" -- swallowed by the CMS,
# so pages still returned 200 with the image silently missing.
@register.filter
