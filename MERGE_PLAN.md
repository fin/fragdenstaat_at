...
| **D** | AT's visual identity | `styles/header.scss`, `homepage.scss`, `blog.scss`, `collapsible.scss`, `cms_utils.scss`, `vega.scss`, `type.scss` | **Keep AT.** This is the genuine 7-file divergence. `donations.scss` was originally listed here but ✅ **taken from DE** (2026-09-11): AT's copy only held a dead `.popin-side` animation, while DE's ships the `.payment-icon` rule that `fds_donation/includes/payment_method.html` (adopted in step 4) depends on — without it the PayPal/Visa SVGs on `/spenden/…/ihre-spenden/` rendered at intrinsic size |
