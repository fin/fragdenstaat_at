...
class BasicDonationForm(StartPaymentMixin, forms.Form):
    ...
    def get_payment_metadata(self, data):
        # AT override: the payee on card/SEPA statements is the association, not
        # the site. Set DONATION_SITE_NAME_OVERRIDE when they differ.
        site_name = getattr(settings, "DONATION_SITE_NAME_OVERRIDE", "") or (
            settings.SITE_NAME
        )
        if data["interval"] > 0:
            return {
                "category": _("Donation for %s") % site_name,
                "plan_name": _(
                ...
                "description": _("Donation for %s") % site_name,
...
COUNTRY_CHOICES = (
    ("", "---"),
    # AT override: Austria first, then its neighbours and German-speaking countries
    ("AT", _("Austria")),
    ("DE", _("Germany")),
    ("CH", _("Switzerland")),
    ("BE", _("Belgium")),
