...
from fragdenstaat_at.theme.search import QueryPreprocessor, get_decompounder_analyzer
from fragdenstaat_at.theme.tests.testdata.search_queries import (
    exact_phrase_search_tests,
    search_tests,
)
from fragdenstaat_at.theme.tests.testdata.search_tokens import (
...
def get_document_classes():
    """The search documents AT actually indexes.

    DE additionally lists ArticleDocument (fds_blog) and froide_evidencecollection's
    EvidenceDocument/PersonDocument. AT installs neither app (D6), and this runs at
    collection time inside @parametrize, so importing them would break collection
    of the whole module rather than skipping a test.
    """
    from froide.foirequest.documents import FoiRequestDocument
    from froide.publicbody.documents import PublicBodyDocument

    from fragdenstaat_at.fds_cms.documents import CMSDocument
...
class TestSearchFilterSet:
    ...
    def test_search(self, decompounder_ready, search, query_text, doc_ids, highlights):
...
class TestElasticsearchAnalyzers:
    ...
    def test_decompounder_analyzer_tokens(self, decompounder_ready, text, tokens):
    ...
    def test_text_analyzer_tokens(self, decompounder_ready, analyze, text, tokens):
...
class TestQueryPreprocessor:
    ...
    def test_prepare_query(self, decompounder_ready, input_text, expected_output):
